...
```python
GimbalOutput(
    pan_delta=-12.5,   # 水平连续旋转舵机速度/方向输入
    tilt_delta=3.0,    # 垂直本轮相对增量，仅供调试
    pan_abs=77.5,      # 水平软件估计角，仅供调试
    tilt_abs=93.0,     # 垂直绝对角度（传给 GPIO 硬件层）
)
```

角度定义：
- `pan_delta` 为本轮水平相对增量，传给连续旋转舵机作为速度/方向输入；`0` 表示停止
- `tilt_abs` 为垂直累计绝对角，限制在 `[0, 180]`，传给绝对位置舵机
- `pan_abs` 为水平软件估计角，范围 `[-180, 180]`，仅供调试
- `tilt_delta` 为本轮垂直相对增量，仅供调试
